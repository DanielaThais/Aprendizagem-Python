"""
INTRODUÇÃO A VARIÁVEIS E CONSTANTES

Contantes são parecidas com variáveis, mas seu valor não pode ser alterado.

Conforme as boas práticas de programção:
Muitas condições no mesmo if(ruim)
    <- Contagem de complexidade (ruim)
"""

# Variáveis são identificadas por letras minúsculas
velocidade = 61     #velocidade atual do carro
local_carro = 101   # local em que o carro está na estrada

# Constantes são identificadas por letras maiúsculas
RADAR_1 = 60        #velocidade máxima do radar 1
LOCAL_1 = 100       #local onde o radar 1 está localizado
RADAR_RANGE = 1     # A distância onde o radar pega

velocidade_acima_radar_1 = velocidade > RADAR_1
carro_passou_radar_1 = local_carro >= (LOCAL_1 - RADAR_RANGE) and local_carro <= (LOCAL_1 + RADAR_RANGE)
carro_multado_radar_1 = carro_passou_radar_1 and velocidade_acima_radar_1

if velocidade_acima_radar_1:
    print('Velocidade do carro passou do radar 1')

if carro_passou_radar_1:
    print('Carro passou no radar 1')

if carro_multado_radar_1:
    print('Carro multado no radar 1')